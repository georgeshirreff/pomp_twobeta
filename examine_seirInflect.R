...
files = list.files(path = "~/Pasteur/tars/output/TOY/", pattern = paste0("s*_", experiment_name, "_.*.csv")
                   , full.names = T)
...
res %>%  write_csv(paste0("~/Pasteur/tars/output/TOY/cat/seirInflect_", experiment_name, ".csv" ))
...
for(Eini in c(1, 3, 10)){
  for(analysis in c("beta2", "beta1profile")){
    ...
    res_piece = read_csv(paste0("~/Pasteur/tars/output/TOY/cat/seirInflect_", exp_name, ".csv" )) %>% mutate(analysis = analysis)
    if(is.null(res)){
      res = res_piece
    } else {
...
itable %>% write_csv(paste0("~/Pasteur/tars/output/Figs/", "inflect_results_table_tinit", ".csv"))
...
ggsave(ggpubr::ggarrange(plotlist = inflect_results_scatter_tinit, ncol = 1)
       , filename = paste0("~/Pasteur/tars/output/Figs/", "inflect_results_scatter_tinit", ".png")
...
for(tini in c("Feb20", "Feb27", "Mar5")){
  for(tinf in c("Mar12", "Mar17", "Mar19")){
    for(analysis in c("beta2", "beta1profile")){
      ...
      res_piece = read_csv(paste0("~/Pasteur/tars/output/TOY/cat/seirInflect_", exp_name, ".csv" )) %>% 
...
source("~/Pasteur/R code/Inflect/seirInflect_source.R")
