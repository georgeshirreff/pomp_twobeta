...
experiment_name = "posneg ALLward Inflect beta2 Einit1 ABStrans tinflect23"
  ...
  for(i in 1:NTESTS){
    ...
    seirInflect %>% 
      mif2(params = seirInflect@params %>% replace(c("beta1", "beta2", "t_inflect", "t_init", "E_init"), c(start_beta1, start_beta2, start_tinflect, start_tinit, start_Einit))
           ...
           # , Np = 10, Nmif = 1000 #seems fine
           # , partrans=parameter_trans(log=c("beta1", "beta2"))
           , partrans = parameter_trans(toEst = Csnippet("\tT_beta1 = beta1;\nT_beta2 = beta2;")
                                        , fromEst = Csnippet("\tbeta1 = fabs(T_beta1);\nbeta2 = fabs(T_beta2);"))
