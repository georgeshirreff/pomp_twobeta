...
# posneg = read_csv(paste0("~/tars/input/posneg_alltests","_", wardName,".csv")) %>%
#   transmute(Date = Date %>% as.numeric, pos, neg, Patients, adm, dd) %>%
#   filter(Date >= as.numeric(as.Date("2020-02-01"))
#          , Date <= as.numeric(as.Date("2020-04-30")))
...
posneg %>% 
  pomp(t0 = posneg$Date[1]
       ...
       , params=c(beta1 = 0.5 # frequency-dependent transmission rate from Is (symptomatic infection)
                  ...
                  , t_inflect_lo = "2020-03-19" %>% as.Date %>% as.numeric
       ...
       , rinit=Csnippet("
         ...
         , StoE = list("double beta = t < t_inflect_lo*(1-hyper) + t_inflect_hi*(hyper) ? beta1 : beta2;
         ...
         , STtoET = list("double beta = t < t_inflect_lo*(1-hyper) + t_inflect_hi*(hyper) ? beta1 : beta2;
...
i = 1
  for(i in 1:NTESTS){
    ...
    print(i)
    print(c(start_beta1, start_beta2, start_hyper, start_tinit, start_Einit))
    ...
    seirInflect %>% 
      mif2(params = seirInflect@params %>% replace(c("beta1", "beta2", "hyper", "t_init", "E_init"), c(start_beta1, start_beta2, start_hyper, start_tinit, start_Einit))
           , Np=mif2_Np, Nmif=Nmif
           # , Np = 10, Nmif = 100 #seems fine
           , partrans=parameter_trans(log=c("beta1", "beta2"), logit = "hyper")
           , paramnames=c("beta1", "beta2", "t_init", "hyper")
    ...
    res_piece <- mif_out %>% coef() %>% bind_rows() %>%
      bind_cols(loglik=ll[1],loglik.se=ll[2], rep = i, seed = updated_seed, wardName = wardName, wardCode = wardCode
                ...
                # , start_tinflect = start_tinflect
                , start_hyper = start_hyper
