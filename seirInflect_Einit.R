...
# b = 1
  ...
  for(i in 1:NTESTS){
    ...
    set.seed(j*b*i + i)
    start_betafactor = runif(1, min = 0.5, max = 1.5)
    # start_tinit = "2020-03-05" %>% as.Date %>% as.numeric
    # start_tinit = "2020-02-27" %>% as.Date %>% as.numeric
    start_tinit = "2020-02-20" %>% as.Date %>% as.numeric
    
    start_tinflect = sample(start_tinit:max(posneg$Date), 1)
    start_Einit = max(1, round(1/runif(1)))
    
    print(i)
    print(c(start_beta1, start_betafactor, start_tinflect, start_tinit, start_Einit))
    ...
    seirInflect %>% 
      mif2(params = seirInflect@params %>% replace(c("beta1", "beta_factor", "t_inflect", "t_init", "E_init"), c(start_beta1, start_betafactor, start_tinflect, start_tinit, start_Einit))
           , Np=mif2_Np, Nmif=Nmif #Np = 10, Nmif = 100 seems fine
           , partrans=parameter_trans(log=c("beta1", "beta_factor", "E_init"))
           , paramnames=c("E_init", "beta1", "beta_factor", "t_inflect")
           , cooling.fraction.50=cooling.fraction
           , rw.sd=rw.sd(beta1 = 0.02, beta_factor = ivp(0.1), t_inflect = ivp(2), E_init=ivp(1))
      ) -> mif_out
    ...
    res_piece <- mif_out %>% coef() %>% bind_rows() %>%
      bind_cols(loglik=ll[1],loglik.se=ll[2], rep = i, seed = updated_seed
                , start_beta1 = start_beta1, start_betafactor, start_tinflect = start_tinflect, start_tinit = start_tinit
    ...
    res %>% write_csv(paste0("output/seirInflect_", experiment_name, "_startbeta1", start_beta1, "_startbetafactor", start_betafactor, "_starttinflect",  start_tinflect, "_starttinit" = start_tinit, "_startEinit", start_Einit , ".csv"))
