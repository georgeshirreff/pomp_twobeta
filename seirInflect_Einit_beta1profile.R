...
experiment_name = paste("posneg Inflect beta1profile"
...
# b = 1
  ...
  for(i in 1:NTESTS){
    ...
    orig_seed = b*100*j^2 + i + start_tinit*start_tinflect
    ...
    seirInflect %>% 
      mif2(params = seirInflect@params %>% replace(c("beta1", "beta2", "t_inflect", "t_init", "E_init"), c(start_beta1, start_beta2, start_tinflect, start_tinit, start_Einit))
           ...
           , partrans=parameter_trans(log=c("beta2", "E_init"))
           , paramnames=c("beta2", "E_init")
           , cooling.fraction.50=cooling.fraction
           , rw.sd=rw.sd(beta2 = ifelse(time >= start_tinflect, 0.02, 0)
                         , E_init=ivp(1))
      ) -> mif_out
