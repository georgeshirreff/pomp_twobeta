...
posneg %>% 
  pomp(t0 = posneg$Date[1]
       ...
       , rprocess=gillespie_hl(
         ...
         , StoE = list("beta = t <= t_inflect ? beta1 : beta2;
         ...
         , STtoET = list("beta = t <= t_inflect ? beta1 : beta2;
...
with(seirInflect@params %>% as.list
     , beta*(psi*(1/gamma*epsilon + 1/(delta)) + kappa1*(1-psi)*(1/(gamma*kappa2)*epsilon + 1/(delta*kappa3))))
...
# experiment_name = "posneg betaProfile Einit tinitMar5 seedFix"
# experiment_name = "posneg betaProfile Einit tinitFeb27 seedFix"
experiment_name = "posneg Inflect betaProfile"
...
# b = 1
  ...
  # res <- read_csv(paste0("output/seirInflect_", experiment_name, "_startbeta", start_beta, "_startEinit", start_Einit , ".csv"))
  # trace <- read_csv(paste0("output/seirInflect_", experiment_name, "_startbeta", start_beta, "_startEinit", start_Einit , "_trace.csv"))
  ...
  for(i in 1:NTESTS){
    ...
    print(i)
    orig_seed = b*100*j + 1e6*i
    updated_seed = orig_seed
    
    
    seirInflect %>% 
      mif2(params = seirInflect@params %>% replace(c("beta", "t_init", "E_init"), c(start_beta, start_tinit, start_Einit))
           , Np=mif2_Np, Nmif=Nmif #Np = 10, Nmif = 100 seems fine
           , partrans=parameter_trans(log=c("E_init"))
           , paramnames=c("E_init")
           , cooling.fraction.50=cooling.fraction
           , rw.sd=rw.sd(E_init=ivp(1))
      ) -> mif_out
    ...
    res %>% write_csv(paste0("output/seirInflect_", experiment_name, "_startbeta", start_beta, "_startEinit", start_Einit , ".csv"))
    # trace %>% write_csv(paste0("output/seirInflect_", experiment_name, "_startbeta", start_beta, "_startEinit", start_Einit , "_trace.csv"))
