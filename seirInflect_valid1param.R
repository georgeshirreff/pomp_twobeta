...
valid_table <- read_csv("input/validation/validation_table_1param.csv")
...
# valid_table <- read_csv("~/Pasteur/tars/input/validation/validation_table_1param.csv")
...
estimate_parameter = valid_table_row$estimate
scale_parameter = valid_table_row$scale
...
if(grepl("^t_", estimate_parameter)){
  transform_parameter = NULL
} else {
  transform_parameter = estimate_parameter
}

if(grepl("^beta", estimate_parameter)){
  proposal_call <- paste0("rw.sd(", estimate_parameter, " = ", scale_parameter, ")")
} else {
  proposal_call <- paste0("rw.sd(", estimate_parameter, " = ivp(", scale_parameter, "))")
}
  ...
  for(i in 1:NTESTS){
    ...
    sims %>% 
      mif2(params = seirInflect@params %>% replace(names(start_parameters), start_parameters)
           ...
           , paramnames=c(estimate_parameter)
