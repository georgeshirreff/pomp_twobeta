...
valid_table <- read_csv("input/validation/validation_table_2param.csv")
...
# valid_table <- read_csv("~/Pasteur/tars/input/validation/validation_table_2param.csv")
...
transform_parameter = c(ifelse(grepl("^t_", estimate1_parameter), "MAKENULL", estimate1_parameter)
  , ifelse(grepl("^t_", estimate2_parameter), "MAKENULL", estimate2_parameter))

transform_parameter = transform_parameter[transform_parameter != "MAKENULL"]


if(grepl("^beta", estimate1_parameter)){
  proposal_call1 <- paste0(estimate1_parameter, " = ", scale1_parameter)
} else {
  proposal_call1 <- paste0(estimate1_parameter, " = ivp(", scale1_parameter, ")")
}

if(grepl("^beta", estimate2_parameter)){
  proposal_call2 <- paste0(estimate2_parameter, " = ", scale2_parameter, "")
} else {
  proposal_call2 <- paste0(estimate2_parameter, " = ivp(", scale2_parameter, ")")
}

proposal_call = paste0("rw.sd(", proposal_call1, ", ", proposal_call2, ")")
  ...
  for(i in 1:NTESTS){
    ...
    sims %>% 
      mif2(params = seirInflect@params %>% replace(names(start_parameters), start_parameters)
           ...
           , paramnames=c(estimate1_parameter, estimate2_parameter)
