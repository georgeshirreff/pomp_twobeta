...
valid_table <- read_csv("input/validation/validation_table_4param.csv")
# posneg = read_csv(paste0("input/validation/posneg_fakeextension.csv")) %>%
posneg = read_csv(paste0("input/validation/posneg_fakeallover.csv")) %>%
  transmute(Date = Date %>% as.numeric, pos, neg, Patients, adm, dd) %>%
  filter(Date >= as.numeric(as.Date("2020-02-01"))
         , Date <= as.numeric(as.Date("2020-05-31")))
# valid_table <- read_csv("~/Pasteur/tars/input/validation/validation_table_4param.csv")
# posneg = read_csv("~/Pasteur/tars/input/validation/posneg_fakeextension.csv") %>%
#   # posneg = read_csv("~/Pasteur/tars/input/posneg_fakeextension.csv") %>%
#   transmute(Date = Date %>% as.numeric, pos, neg, Patients, adm, dd) %>%
#   filter(Date >= as.numeric(as.Date("2020-02-01"))
#          , Date <= as.numeric(as.Date("2020-05-31")))
...
proposal_call = c("rw.sd(beta1 = 0.02, beta_factor = 0.02, E_init = ivp(1), t_init = ivp(0), t_inflect = ivp(1))")
