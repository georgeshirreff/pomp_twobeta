...
ward = commandArgs(trailingOnly = T)[2]


wardCode = case_when(ward == "Galien_2" ~ "A2"
                     , ward == "Hamburger_3" ~ "B3"
                     , ward == "Rabelais_0" ~ "C0"
                     , ward == "Rabelais_2" ~ "C2")

wardSeed = case_when(ward == "Galien_2" ~ 7^3
                     , ward == "Hamburger_3" ~ 7^4
                     , ward == "Rabelais_0" ~ 7^5
                     , ward == "Rabelais_2" ~ 7^6)

posneg = read_csv(paste0("input/posneg_alltests","_", ward,".csv")) %>%
...
# posneg = read_csv(paste0("~/Pasteur/tars/input/posneg_alltests","_", ward,".csv")) %>%
#   transmute(Date = Date %>% as.numeric, pos, neg, Patients, adm, dd) %>%
#   filter(Date >= as.numeric(as.Date("2020-02-01"))
#          , Date <= as.numeric(as.Date("2020-04-30")))
...
experiment_name = "posnegWard Inflect beta1profile Einit1"
...
# b = 1
  ...
  for(i in 1:NTESTS){
    ...
    res_piece <- mif_out %>% coef() %>% bind_rows() %>%
      bind_cols(loglik=ll[1],loglik.se=ll[2], rep = i, seed = updated_seed, ward = ward, wardCode = wardCode
